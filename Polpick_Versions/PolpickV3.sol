...
contract PolPick is Ownable {
  ...
  address public gameController15;
  address public gameController30;
...
event TradePlaced(bytes poolId, address sender, uint256 amount, string prediction, uint256 newTotal, bytes indexed indexedPoolId, address indexed indexedSender, string avatarUrl, string countryCode, int64 roundStartTime, string whiteLabelId,string gameId);
  ...
  constructor(address _newGameController15, address _newGameController30) Ownable(_newGameController15) {
    gameController15 = _newGameController15;
    gameController30 = _newGameController30;
  }
 ...
 modifier onlyOpenPool (bytes calldata poolId) {
  ...
  modifier onlyGameController () {
    require(msg.sender == gameController15 || msg.sender == gameController30, 'Only game controller can do this');
    _;
  }
 ...
 function addBet (
   ...
   function makeTrade(
    ...
    require(isEOA(msg.sender) || allowedContracts[msg.sender], "must be EOA or allowed contract");
    require(msg.value > 0, "value must be greater than zero");
    require(msg.value >= pools[userTrade.poolId].minBetAmount, "amount should be higher than the minimum");
    require(msg.value <= pools[userTrade.poolId].maxBetAmount, "amount should be lower than the maximum");
    ...
    userTrade.poolId, msg.sender, userTradeData.avatar, userTradeData.countryCode, userTradeData.roundStartTime, userTradeData.whiteLabelId,userTradeData.gameId);
  ...
  function sendEther (
   ...
   //Function to send amount from contact address
   function transferEther(address payable _to, uint256 _amount) public onlyOwner {
     ...
     // Methods
  function changeGameControllerAddress15(address newGameControllerAddress) public onlyOwner  {
    gameController15 = newGameControllerAddress;
  }
  function changeGameControllerAddress30(address newGameControllerAddress) public onlyOwner  {
    gameController30 = newGameControllerAddress;
  }

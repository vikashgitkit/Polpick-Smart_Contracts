 ...
 modifier onlyOpenPool (bytes calldata poolId) {
  ...
  function trigger(
    ...
    if(isPoolOpen(poolId)) { 
      console.log("Inside isPoolOpen trigger L109...");
      ...
      console.log("Inside endprice==0 L119....");
  ...
  function distribute (
    ...
    if (round.upBetGroup.bets.length == 0 || round.downBetGroup.bets.length == 0 || (round.startPrice == round.endPrice)) 
        console.log("Inside Distribute checking length L155", round.upBetGroup.bets.length, round.downBetGroup.bets.length );   
    {
      if (round.startPrice == round.endPrice){ //In case of TIE return the investments to both sides
        console.log("If round is tie L158:");
        ...
        uint fromReturnUp = returnGroupUp.distributedCount;
         console.log("Up counted L163:",fromReturnUp);
        uint fromReturnDown = returnGroupDown.distributedCount;
        console.log("Down counted L165:",fromReturnDown);
        ...
        if (returnGroupUp.distributedCount == returnGroupUp.bets.length && returnGroupDown.distributedCount == returnGroupDown.bets.length) {
         console.log("Inside befor clearPool L173......");
          clearPool(poolId);
        }
        ...
        console.log("From return L181......", fromReturn);
        ...
        if (returnGroup.distributedCount == returnGroup.bets.length) {
          console.log("Inside befor clear pool L186......");
          clearPool(poolId);
        }
      }
    ...
    if (round.startPrice < round.endPrice) {
      winners = round.upBetGroup;
        console.log("If start Price is leass than end price L198......");
  ...
  function clearPool (
    ...
    console.log("Inside clearPool L262.....");
 ...
 function addBet (
  ...
  function makeTrade(
    ...
    if (userTrade.upOrDown) {
     ...
     console.log("Up trade users L326:", newTotal);
    } else {
      ...
      console.log("Down trade users L330:", newTotal);
    }
  ...
  function sendEther (
    ...
    // New receive function to handle incoming Ether transactions
    receive() external payable {    }


     // Methods
  function changeGameControllerAddress(address newGameControllerAddress) public onlyOwner nonZeroAddress(newGameControllerAddress) {
  ...
  function changeGameFeeAddress(address newFeeAddress) public onlyOwner nonZeroAddress(newFeeAddress){
   // require(newFeeAddress != address(0x0) , "Address cannot be zero address");

    feeAddress = newFeeAddress;
  }

  function changeGameFeeJackpotAddress(address newFeeJackpotAddress) public onlyOwner nonZeroAddress(newFeeJackpotAddress){
  ...
  modifier nonZeroAddress(address newAddress) {
